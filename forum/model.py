...
class UserObj(db.Model):
  ...
  mailing = db.BooleanProperty(default=False)
  
  #@property
  def googleTalkStatus(self):
    try:
      status = xmpp.get_presence(self.login.email())
    except:
      status = False
    return status
...
class Topic(db.Model, DataBaseOperations):
  ...
  def getUserList(self):
    """return users list who wrote somethink in this topic"""
    userList = []
...
class Post(db.Model, DataBaseOperations):
  ...
  def sendMailToAll(self, sender, additionalText):
    userList = self.topic.getUserList()
    toEmails = []
