...
public class ElementRecordDO implements Comparable<ElementRecordDO>, Comparator<GameMetaCardDO> {
    ...
    @NotPersistent
    private SortedSet<GameMetaCardDO> dealtCards;
    ...
    public ElementRecordDO(GameElementSchoolDO elementSchool) {
        ...
        this.dealtCards = new TreeSet<GameMetaCardDO>();
    }
    ...
    public void restore(BoardManagerDO boardManager) {
        ...
        dealtCards = new TreeSet<GameMetaCardDO>(this);
        for (Key key : dealtCardKeys) {
            dealtCards.add(pm.getObjectById(GameMetaCardDO.class, key));
        }
    }
    ...
    public GameMetaCardDO[] getDealtCards() {
        return dealtCards.toArray(new GameMetaCardDO[dealtCardKeys.size()]);
    }

    public boolean addDealtCard(GameMetaCardDO card) {
        return dealtCardKeys.contains(card.getKey()) && dealtCards.add(card) && dealtCardKeys.add(card.getKey());
    }

    public boolean removeDealtCard(GameMetaCardDO card) {
    ...
    public int compare(GameMetaCardDO card1, GameMetaCardDO card2) {
